...
nmap_tcp_fast(){
    ...
    if [[ ! -f ${folder}/tcp.nmap ]]; then
        ...
        if grep /open/ ${folder}/tcp.gnmap; then
            notify-send -t 7000 "Open ports at ${ip}" "$(grep -E -o '[0-9]+/open/' ${folder}/tcp.gnmap)"
        fi
    fi
}
nmap_udp_20(){
    ...
    if [[ ! -f ${folder}/udp.nmap ]]; then
        notify-send -t 5000 "Scanning ${ip}..."
        sudo $NMAP -sUVC --packet-trace \
             --top-ports=20 \
             -oA ${folder}/udp --max-retries=0 --reason -n -F -Pn ${ip}
        if grep /open/ ${folder}/udp.gnmap; then
            notify-send -t 7000 "Open ports at ${ip}" "$(grep -E -o '[0-9]+/open/' ${folder}/udp.gnmap)"
        fi
    fi
}
nmap_tcp_full(){
